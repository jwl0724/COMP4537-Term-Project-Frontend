...
//             const response = await fetch('http://127.0.0.1:8080/api/v1/forgotPassword', {
//                 method: 'POST',
//                 headers: {
...
// forgotPassword.js
import { APIHub } from './classes/apiHub.js';  // Adjust path based on folder structure
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    form.addEventListener('submit', async (e) => {
        ...
        submitButton.disabled = true; // Disable the submit button
        try {
            const response = await APIHub.forgotPassword(email); // Use the APIHub class
            ...
            if (response.success) {
                ...
                // Show error message from backend
                errorMessage.textContent = response.error || 'Something went wrong. Please try again later.';
                errorMessage.style.color = 'red';
            }
        } catch (error) {
            // Network error handling
